...
ll dfs(vector<vector<ll>>& matrix, int i, int j)
{
 int m = matrix.size();
 int n = matrix[0].size();
...
ll fn( vector<vector<ll>>& matrix, int m, int n){
 ll res=0;
 ll mx = -1;
 rep(i,0,m){
    rep(j,0,n){
 ...
 rep(i,0,m){
  rep(j,0,n){
   if( matrix[i][j] == mx ) maxI.push_back({i,j});
  }
 }
 for(pair<int,int>& p : maxI)
 {
  ll ans = 0;
  ans = dfs( matrix, p.first, p.second );
  res += ans;
 }
 return res;
}
